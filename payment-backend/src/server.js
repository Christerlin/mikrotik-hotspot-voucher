...
//     secret ; endpoint strictement rate-limité contre le brute force.
app.get("/api/retrieve/:pin", rateLimit(15), async (req, res) => {
  try {
    const pin = String(req.params.pin || "").toUpperCase().replace(/\s+/g, "");
    const order = pin ? await getOrderByPin(pin) : null;
    if (!order) return res.status(404).json({ error: "Code introuvable." });
    if (order.status === "PENDING") {
      const last = lastVerify.get(order.reference) || 0;
