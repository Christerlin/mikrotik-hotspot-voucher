...
export const config = {
  ...
  // Où renvoyer le client après le paiement (Return URL configurée chez Pay'm).
  // Pay'm redirige vers /return sur CE backend (URL publique HTTPS), qui renvoie
  // ensuite le client vers le portail captif ci-dessous.
  portalReturnUrl: process.env.PORTAL_RETURN_URL || "http://lambda.connect/prix.html",
  // Durée de vie du jeton de passation remis au portail par /return.
  handoffTtlMs: Number(process.env.HANDOFF_TTL_MS || 15 * 60 * 1000), // 15 min
  // Temps max que /return accorde à la base avant de rediriger sans jeton.
  // Le client vient de payer : on ne le fait jamais attendre la base.
